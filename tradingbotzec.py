...
class TrendFollowingStrategy:
    def __init__(self, pair: str = "ZEC/USD", initial_balance: float = 1000):
        self.pair = pair
        self.base_currency = pair.split('/')[0]  # ZEC
    ...
    def get_current_price(self) -> Optional[float]:
        ...
        if ticker:
            return ticker.get('Data',{}).get('ZEC/USD',{}).get('LastPrice')
    ...
    def run_strategy(self, check_interval: int = 60):  # 5分鐘檢查一次
        """運行策略"""
        count=0
...
if __name__ == "__main__":
    ...
    # 運行策略 (每5分鐘檢查一次)
    strategy.run_strategy(check_interval=60)
